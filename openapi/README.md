...
- `en/aitoearn.openapi.json`: English AiToEarn Open Platform API spec generated from the Chinese target spec through `spec-translations.en.json`.
...
- `spec-translations.en.json`: Exact Chinese-to-English translation dictionary used by `scripts/sync-openapi-docs.mjs` to derive the English OpenAPI spec. If a future sync introduces new Chinese strings, add the translations here before regenerating.
...
- Do not edit `en/aitoearn.openapi.json` directly. Update `spec-overrides.json` for Chinese source content, update `spec-translations.en.json` for English wording, then regenerate via `node scripts/sync-openapi-docs.mjs`.
