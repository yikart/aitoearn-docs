(function () {
  ...
  function isRequestHeaderTitle(element: Element) {
    return REQUEST_HEADER_TITLES.has(getText(element))
  }

  function hasOtherApiSectionTitle(container: Element, targetTitle: Element) {
  ...
  function findRequestHeaderContainer(title: Element) {
    ...
    while (current && current !== document.body) {
      ...
      if (
        text.length <= MAX_SECTION_TEXT_LENGTH &&
  ...
  function resetHiddenSections() {
    document.querySelectorAll<HTMLElement>(`[${HIDDEN_ATTRIBUTE}="true"]`).forEach((element) => {
